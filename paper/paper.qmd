...
model <- lm(
  formula = `Affordability Index` ~ `Interest Rate (%)` + `CPI (Index)`, 
  data = analysis_data
)
...
$$y_i = y_i = \beta_0 + \beta_1 \cdot \mathrm{CPI}_i + \beta_2 \cdot \mathrm{House    Supply}_i + \epsilon_i
$$

Where:

-   $y_i$ is the affordability index for observation iii, which measures the ratio of house prices to average wa ges, indicating housing affordability.
...
-   $\beta_2$ captures the effect of the Interest Rate on the affordability index.
